...
} from "../chunk-3GJGYFSZ.mjs";
import "../chunk-3UDJ4GFA.mjs";
import {
  task
} from "../chunk-O6O67SVP.mjs";
import "../chunk-WADMXOVM.mjs";
import {
  logger
} from "../chunk-YXBRSIPQ.mjs";
...
// trigger/design-agent.ts
var AI_USER_ID = "ghost-ai-design-agent";
var AI_USER_INFO = {
  name: "Ghost AI",
  avatar: "",
  color: "#8b5cf6"
...
var GRID_GAP_Y = 150;
var MAX_AI_NODES = 12;
var STATUS_PRESENCE_TTL_SECONDS = 60;
var designPlanSchema = {
  type: "object",
...
function normalizeDesignPlan(value) {
  const parsed = value;
...
__name(normalizeDesignPlan, "normalizeDesignPlan");
function parseDesignPlan(text) {
...
__name(parseDesignPlan, "parseDesignPlan");
function createFallbackDesignPlan(prompt, canvas) {
  ...
  const summary = prompt.trim().length > 0 ? `Created a SaaS product canvas for: ${prompt.trim().slice(0, 90)}` : "Created a SaaS product canvas from a fallback plan.";
  return {
    summary,
    actions: [
      {
...
__name(createFallbackDesignPlan, "createFallbackDesignPlan");
function isDesignAction(value) {
  ...
  if (action.type === "moveNode") {
    return typeof action.id === "string" && typeof action.x === "number" && typeof action.y === "number";
  }
  if (action.type === "resizeNode") {
    return typeof action.id === "string" && typeof action.width === "number" && typeof action.height === "number";
  }
...
__name(isDesignAction, "isDesignAction");
function sanitizeId(value, fallback) {
  const sanitized = value.trim().toLowerCase().replace(/[^a-z0-9_-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 64);
...
__name(normalizeShape, "normalizeShape");
function normalizePalette(color, textColor, index) {
...
__name(normalizePalette, "normalizePalette");
function createNodeFromAction(action, index, existingIds) {
  ...
  return {
    ...
    type: "canvas",
    position: {
      x: clampNumber(action.x, column * GRID_GAP_X, -2e3, 4e3),
      y: clampNumber(action.y, row * GRID_GAP_Y, -2e3, 4e3)
    },
    width,
    height,
...
__name(createNodeFromAction, "createNodeFromAction");
function createEdgeFromAction(action, index, existingIds) {
...
__name(createEdgeFromAction, "createEdgeFromAction");
...
async function readCanvas(roomId) {
...
async function publishStatus(roomId, kind, message) {
  try {
...
async function updateAiPresence(roomId, cursor, thinking, ttl = STATUS_PRESENCE_TTL_SECONDS) {
  try {
...
__name(updateAiPresence, "updateAiPresence");
async function interpretPromptWithGemini(prompt, canvas) {
...
__name(interpretPromptWithGemini, "interpretPromptWithGemini");
async function applyDesignPlan(roomId, plan) {
      ...
      for (const action of plan.actions) {
        ...
        if (action.type === "moveNode" && nodeIds.has(action.id)) {
          flow.updateNode(action.id, {
            position: {
              x: clampNumber(action.x, 0, -2e3, 4e3),
              y: clampNumber(action.y, 0, -2e3, 4e3)
            }
          });
          appliedActions += 1;
          continue;
        ...
        if (action.type === "deleteNode" && nodeIds.has(action.id)) {
          const connectedEdges = flow.edges.filter((edge) => edge.source === action.id || edge.target === action.id).map((edge) => edge.id);
        ...
        if (action.type === "addEdge" && nodeIds.has(action.source) && nodeIds.has(action.target) && action.source !== action.target) {
...
__name(applyDesignPlan, "applyDesignPlan");
  ...
  run: /* @__PURE__ */ __name(async (payload) => {
    ...
    try {
      ...
      await publishStatus(
        payload.roomId,
        "complete",
        appliedActions > 0 ? `Ghost AI updated the canvas: ${plan.summary}` : "Ghost AI finished but did not find any safe canvas updates to apply."
      );
