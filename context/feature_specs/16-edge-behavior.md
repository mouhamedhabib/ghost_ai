...
- Added four subtle node connection handles that fade in on node hover
  or selection.
- New connections now create custom canvas edges with rounded
  right-angle routing and arrowheads.
- Custom edges support wide invisible hit targets, active hover/selection
  styling, and inline label editing through the collaborative edge data
  flow.
